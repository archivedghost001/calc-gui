...
# Tkinter setup
therotconsumes = Tk()
therotconsumes.geometry("550x270")
therotconsumes.title("Calculator")

# Setting icon for the Application
photo = PhotoImage(file="G.png")
therotconsumes.iconphoto(False, photo)
...
# History Panel
historySide = Frame(therotconsumes, bg=rgb_to_hex(historyPanelBackground))
...
Button(
    ...
    borderwidth=0,
    command=saveCurrentInputToHistory,
).pack(side=RIGHT)
...
# Call the var change once
# so it its evaluated without actual change
varChange("WOOOOOO LASERS BE UPON YEE!!!!")
...
# Quit command
filemenu.add_command(label="Exit", command=therotconsumes.quit)

menubar.add_cascade(menu=filemenu, label="Formulas")

therotconsumes.config(menu=menubar)

therotconsumes.mainloop()
